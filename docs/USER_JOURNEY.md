...
## Journey 1: Whole bundle from a remote
...
from any harness. Commit and push from the repo when changes are ready.
...
`caveman` stays a copied `url` skill because only local sources link.
